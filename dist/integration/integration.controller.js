...
let IntegrationController = class IntegrationController {
    ...
    idempotencyService;
    dbService;
    constructor(batchSyncService, balanceRepo, auditService, idempotencyService, dbService) {
    ...
    singleBalanceUpdate(req, dto) {
        ...
        const result = this.dbService.runInTransaction(() => {
            ...
            if (!local) {
                ...
                this.balanceRepo.updateFromHcm(dto.employee_id, dto.leave_type, dto.total_balance, dto.used_balance, dto.hcm_version, local.version);
            }
